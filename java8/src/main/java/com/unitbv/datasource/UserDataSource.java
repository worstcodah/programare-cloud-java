...
public class UserDataSource {
    ...
    public String getJobOfTheOldestUser() {
        ...
        return users.stream().sorted(Comparator.comparingInt(User::getAge).
                reversed()).findFirst().get().getJob();
    }

    // Get user (distinct) jobs sorted alphabetically
    public Set<String> getAllUserJobsSorted() {
        return users.stream().map(u -> u.getJob()).collect(Collectors.toSet());
    }

    // Find user by first name - throw RuntimeException if not found
    public User findByFirstName(String firstName) {
        return users.stream().filter(u -> u.getFirstName().equals(firstName)).findFirst().get();
    }
    ...
    public void addUser(User user) {
        ...
        users.stream().filter(u -> u.getId() == user.getId()).findFirst().ifPresentOrElse((u) ->
                {
